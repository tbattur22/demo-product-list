  ...
  const loadMore = (entry) => {
    ...
    if (products.length < total && !isFirstTimeRef.current && search.length === 0) {
      Product.getProducts(products, limit).then((loadedProducts) => {
        ...
        if (loadedProducts?.products[0]?.title) {
          setProducts([...products, ...loadedProducts.products]);
          setTotal(loadedProducts.total);
        }
      });
    // } else {
    //   console.log(`loadMore() called but condition not met, cur products:isFirstTime`);
    }
  };
